...
@section('schema')
@php
    $multiWordBrands = ['Rolls Royce', 'Rolls-Royce', 'Land Rover', 'Range Rover', 'Aston Martin', 'Mercedes Benz', 'Mercedes-Benz'];
    $brandName = null;
    foreach ($multiWordBrands as $b) {
        if (\Illuminate\Support\Str::startsWith($fleet->name, $b)) { $brandName = $b; break; }
    }
    $brandName = $brandName ?: \Illuminate\Support\Str::before($fleet->name, ' ');
@endphp
<script type="application/ld+json">
    {
        ...
        @if((float) $fleet->price > 0)
        ,"offers": {
            "@@type": "Offer",
            "url": "{{ url()->current() }}",
            "priceCurrency": "GBP",
            "price": "{{ number_format((float) $fleet->price, 2, '.', '') }}",
            "availability": "https://schema.org/InStock",
            "seller": { "@@id": "{{ rtrim(url('/'), '/') }}/#organisation" }
        }
        @endif
    }
</script>
...
@section('content')
        ...
        <div class="container">
            ...
            <div class="top-content">
                <div class="left">
                    ...
                    <h1>{{ $fleet->name }} Hire in London</h1>
                    ...
                    <div class="fleet-price">
                        @if ($fleet->price == 0)
                            <div class="fleet-price-value"> POA </div>
                        @else
                            <small>Daily Price</small>
                            <div class="fleet-price-value">£ {{ number_format($fleet->price, 2) }}</div>
                        @endif
                ...
                @if(($comparison ?? collect())->count() > 1)
                    <div class="fleet-compare">
                        ...
                        <div class="fleet-compare-scroll">
                            <table>
                                <thead>
                                    <tr><th>Vehicle</th><th>Daily hire from</th></tr>
                                </thead>
                                <tbody>
                                @foreach($comparison as $row)
                                    <tr @class(['is-current' => $row->id === $fleet->id])>
                                        <td>
                                            @if($row->id === $fleet->id)
                                                <strong>{{ $row->name }}</strong> &mdash; this page
                                            @else
                                                <a href="{{ url('/fleet/' . $row->slug) }}">{{ $row->name }}</a>
                                            @endif
                                        </td>
                                        <td>{{ (float) $row->price > 0 ? '£' . number_format($row->price, 2) : 'POA' }}</td>
                                    </tr>
                                @endforeach
                                </tbody>
                            </table>
        ...
        <div class="modal-dialog modal-dialog-centered modal-xl">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="car-title" id="fleetTitle"></div>
    ...
    @include("front.seo.partials.fleet-content", ["fleet" => $fleet])
    @include("front.seo.partials.fleet-links", ["fleet" => $fleet])
