...
public function fleet_detail($slug){
    $fleet = Product::where('slug', $slug)
        ...
        $base = $fleet->name . ' Hire London';
        $metaTitle = strlen($base . ' | Suave Executive Travel') <= 60
            ? $base . ' | Suave Executive Travel'
            : $base . ' | Suave';
    $body = trim(preg_replace('/\s+/', ' ', strip_tags($fleet->description)));
    $metaDescription = Str::limit($fleet->name . ' hire in London and the Thames Valley. ' . $body, 155, '...');
